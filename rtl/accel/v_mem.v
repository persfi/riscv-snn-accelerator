...
//word_cnt must stay inside the active layer's range (0..31 for layer 1, 0..2 for layer 2). 
...
module v_mem (
    ...
    always @(posedge clk) begin
        if(ctrl == V_CLEAR) begin
            ...
            for (i = 0; i < LANES; i = i + 1) begin
                v[base+word_cnt*LANES +i] <= v_wdata[i*16+:16];
            end
        end
    end
    ...
    always @(*) begin
            for (j = 0; j < LANES; j = j + 1) begin
                v_rdata[j*16+:16] = v[base + word_cnt*LANES + j];
            end
    end
